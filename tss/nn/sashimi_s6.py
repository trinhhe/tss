"""
SaShiMi backbone from https://github.com/state-spaces/s4/blob/main/models/sashimi/sashimi.py
We replace S4+MLP with Mamba block as described in https://arxiv.org/abs/2312.00752 (Mamba: Linear-Time Sequence Modeling with Selective State Spaces)
...
from tss.nn.s4 import S4Block as S4
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        z = self.layer(z)
        z = z.transpose(-1, -2)
        # z, _ = self.layer(z) # when using S4 also can output state
    ...
    def default_state(self, *args, **kwargs):
        if not isinstance(self.layer, Mamba):
    ...
    def step(self, x, state, **kwargs):
        ...
        if not isinstance(self.layer, Mamba):
            ...
            z, state = self.layer.step(z, ssm_state=state)
        # Residual connection
        x = z + x
...
class Sashimi(nn.Module):
    def __init__(
        ...
        d_model=64,
        n_layers=8,
        pool=[4, 4],  # TODO: in Mamba paper they do p = 16
        expand=2,
        ff=2,
        bidirectional=False,
        unet=False,
